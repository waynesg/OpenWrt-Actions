...
"
	...
	;;
	u)
		# autoupdate v2 (preferred): /usr/libexec/autoupdate (integrated by luci-app-autoupdate branch 2.0)
		if [ -x /usr/libexec/autoupdate ]; then
			ECHO y "\n请前往 LuCI: 系统 -> AutoUpdate 进行更新，或执行: /usr/libexec/autoupdate upgrade"
			sleep 2
		elif [[ -s ${AutoUpdate_File} ]]; then
			# legacy fallback (older firmwares)
			AutoUpdate_UI
		else
			ECHO r "\n未检测到更新程序：/usr/libexec/autoupdate 或 /bin/AutoUpdate.sh"
			sleep 2
		fi
	;;
	x)
