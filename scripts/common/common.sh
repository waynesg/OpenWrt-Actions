...
cd ${GITHUB_WORKSPACE}
# IMPORTANT: clone the same branch that is running this workflow, otherwise we overwrite branch history
# (e.g. running on 2.0 but cloning main would force-push main onto 2.0)
git clone -b ${GITHUB_REF_NAME} https://github.com/${Apidz}.git ${FOLDER_NAME}
...
git push "https://${REPO_TOKEN}@github.com/${Apidz}" HEAD:${GITHUB_REF_NAME}
}
################################################################################################################
# LEDE源码通用diy.sh文件
################################################################################################################
...
if [[ ${REGULAR_UPDATE} == "true" ]]; then
        # autoupdate v2 (x86_64): use luci-app-autoupdate branch 2.0, no legacy /bin/AutoUpdate.sh chain
        git clone -b 2.0 https://github.com/waynesg/luci-app-autoupdate feeds/luci/applications/luci-app-autoupdate
	# keep tools that are still useful; drop legacy updater scripts
	cp -Rf "${PATH1}"/{AutoBuild_Tools.sh,tv.sh} package/base-files/files/bin
fi
